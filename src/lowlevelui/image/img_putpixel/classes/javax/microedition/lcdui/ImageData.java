...
final class ImageData implements AbstractImageData {
    ...
    ImageData(int width, int height, boolean isMutable,
        ...
        // We don't use ImageDataFactory.bytesInPixel() since current format
        // may changed since the time the pixelData array was created
        this.bytesInPixel = ((pixelData.length / (width * height)) < 3) ? 2 : 4;
    ...
    synchronized void checkPixelFormat() {
        // Skip format check for romized images b/c we are handled them in a
        // different way
        if (pixelData == null) {
            return;
        }
